...
def get_grounding_output(model, image, caption, box_threshold, text_threshold=None, 
                         with_logits=True, cpu_only=False, token_spans=None,get_one_mask=True):
...
def main_run(config_file, checkpoint_path, image_path, text_prompt, output_dir, box_threshold=0.3, text_threshold=0.25, token_spans=None, cpu_only=False,get_one_mask = True):
    # make dir
    os.makedirs(output_dir, exist_ok=True)
    # load image
    image_pil, image = load_image_square(image_path)
    ...
    # run model
    boxes_filt, pred_phrases = get_grounding_output(
        model, image, text_prompt, box_threshold, text_threshold, cpu_only=cpu_only, token_spans=eval(f"{token_spans}"),get_one_mask=get_one_mask
    )
